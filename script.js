...
function initializeDashboard() {
    const checklists = ["frontend", "backend", "cloud", "data", "devops", "mobile", "security", "aiml"];
...
function updateStats() {
    ...
    checklists.forEach(checklist => {
        ...
        criticalItems += Object.entries(states)
            .filter(([key, value]) => key.includes("critical") && !value).length;
    });
...
function exportToCSV() {
    ...
    const rows = checklists.map(checklist => [
        checklist,
        loadChecklistProgress(checklist)
    ]);
