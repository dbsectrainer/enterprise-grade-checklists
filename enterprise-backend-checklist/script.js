document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("reset-checklist").addEventListener("click", () => {
    if (confirm("Are you sure you want to reset all checklist items? This cannot be undone.")) {
      ...
      localStorage.removeItem("backendChecklistStates");
      localStorage.removeItem("backendLastUpdated");
  ...
  function saveCheckboxStates() {
    ...
    localStorage.setItem("backendChecklistStates", JSON.stringify(states));

    // Update last updated timestamp
    const timestamp = new Date().toISOString();
    localStorage.setItem("backendLastUpdated", timestamp);

    // Trigger storage event for dashboard update
    const event = new Event("storage");
    event.key = "backendChecklistStates";
    window.dispatchEvent(event);
  }

  // Load checkbox states from localStorage
  function loadCheckboxStates() {
    const states = JSON.parse(localStorage.getItem("backendChecklistStates"));
    if (states) {
      document.querySelectorAll(".checkbox").forEach((checkbox, index) => {
